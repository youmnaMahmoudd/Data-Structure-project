...
using namespace std;
QPixmap Graph_present(string xml_file);
...
QPixmap Graph_present(string xml_file)
{
    ...
    string my_graph = "digraph G{subgraph cluster_p {label = Graph;node [shape=record fontname=Arial color = deeppink4];edge [color = black];";
    ...
    for (int i = 0; i < xml_file.length(); i++)
    {
        ...
        else
        {
            ...
            user_id_name = user_id + "[ style=filled, fillcolor=gray100 ,label=\"{name = " + user_name + "|id = " + user_id + "}\"] ";
    ...
    QString text = QString::fromStdString(my_graph);
    out << text;
    file.flush();
    file.close();
    system("C:\\Users\\Bahaa\\Desktop\\Graphviz\\bin\\dot -Tpng -O graph.dot");
